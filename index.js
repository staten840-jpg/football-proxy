...
function openModal(home, away) {
  ...
  fetch("https://api.anthropic.com/v1/messages",{
    ...
    body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:`${home} vs ${away} o'yini uchun SofaScore uslubida qisqa tahlil bering. O'zbek tilida.\n\n⚽ G'olib ehtimoli\n📊 Eng ehtimoliy natija\n🎯 Ikki jamoa gol uradi\n📝 Qisqa xulosa`}]})
  }).then(r=>r.json()).then(data=>{
    document.getElementById("modal-content").className="modal-content";
    document.getElementById("modal-content").textContent=data.content?.[0]?.text||"Tahlil olinmadi";
  }).catch(()=>{
    document.getElementById("modal-content").className="modal-content";
